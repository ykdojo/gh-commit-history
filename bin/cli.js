...
Options:
  ...
  -r, --repo <name>    Single-repo view: just this repo's commits ("name" assumes
                       your account, or pass "owner/name")
...
<div class="container">
  ...
  <div class="footer">Generated by <a href="https://github.com/ykdojo/gh-commit-history" target="_blank" rel="noopener">ykdojo/gh-commit-history</a> &nbsp;|&nbsp; <code>${cmd}</code></div>
</div>
<script>
...
function applyZoom(){
  ...
  Plotly.relayout('chart',r);if(!D.singleRepo)Plotly.relayout('repo-chart',r);
}
...
function renderAll(){updateSubtitle();renderTotal();if(!D.singleRepo){renderRepos();renderTotals();}applyZoom();}
...
function main() {
  ...
  // Single-repo view: "name" assumes the resolved account; "owner/name" is used as-is.
  const repoFull = opts.repo ? (opts.repo.includes('/') ? opts.repo : `${login}/${opts.repo}`) : null;
  const repoKey = repoFull ? repoFull.toLowerCase() : null;
  if (repoFull) console.log(`Single-repo view: ${repoFull}`);
  ...
  for (const [repo, obj] of Object.entries(allRepoDaily)) {
    ...
    if (repoKey && repo.toLowerCase() !== repoKey) continue; // single-repo view
    const o = {};
  ...
  // Single-repo view: tighten the timeline to the repo's own lifetime instead of
  // the account's full history, so its activity fills the chart rather than
  // showing up as a sliver at the far right.
  let displayStart = rangeStart;
  if (repoFull) {
    const dates = Object.keys(daily);
    if (dates.length) displayStart = dates.reduce((a, b) => (a < b ? a : b));
    else console.warn(`  No commits found for ${repoFull} (by ${login}). Check the owner/name.`);
  } else {
  ...
  const defaultName = repoFull ? `${login}-${repoFull.split('/').pop()}.html` : `${login}.html`;
